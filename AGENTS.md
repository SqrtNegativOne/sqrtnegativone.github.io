...
- **Hosting**: Currently built with `@sveltejs/adapter-static` and deployed to **GitHub Pages** via `.github/workflows/deploy.yml` on push to `main`. A migration to **Cloudflare Workers** (`@sveltejs/adapter-cloudflare` + D1) is staged but **not active** — see "Cloudflare migration (staged)" below.
...
- `wrangler.jsonc` - Cloudflare Workers config (staged): assets directory, compatibility flags, and the D1 binding `DB`.
- `src/worker-configuration.d.ts` - Generated by `bunx wrangler types src/worker-configuration.d.ts` (staged). Declares the `Env` interface and Worker runtime types; re-run after changing `wrangler.jsonc`.
- `.github/workflows/deploy-cloudflare.yml` - Manual-only Cloudflare deploy (staged). Not triggered by push.
...
**Cloudflare Commands (staged migration — only apply after flipping):**
- `bunx wrangler dev` - Runs the built Worker locally with emulated bindings.
- `bunx wrangler deploy` - Deploys the Worker and static assets to Cloudflare.
- `bunx wrangler types src/worker-configuration.d.ts` - Regenerates `src/worker-configuration.d.ts` from `wrangler.jsonc`. Re-run after changing bindings.
- `bunx wrangler d1 execute sqrt-fyi --local --file=schema.sql` - Apply SQL to the local D1 database (drop `--local` for `--remote`).
...
- **Hybrid rendering (post-migration)**: Once deployed to Cloudflare, `src/routes/+layout.js` sets `prerender = true`, so all existing pages stay static. Any new route that needs the server (form actions, D1, auth) must opt out with `export const prerender = false` in its `+page.server.ts`/`+page.ts`. Keep static pages prerendered for speed and SEO.
...
## Cloudflare migration (staged)
The repo already contains the Cloudflare scaffolding but still deploys to GitHub Pages. To flip:
1. In `svelte.config.js`, switch `adapter-static` → `@sveltejs/adapter-cloudflare` (with the `platformProxy` option for local D1 emulation).
2. Create the D1 database (`bunx wrangler d1 create sqrt-fyi`) and put its `database_id` in `wrangler.jsonc`.
3. Add repo secrets `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID`.
4. Give `.github/workflows/deploy-cloudflare.yml` a `push` trigger, remove/disable `.github/workflows/deploy.yml`, disable Pages in repo settings, and point `sqrt.fyi` at the Worker.