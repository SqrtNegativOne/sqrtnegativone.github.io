...
<div class="space-y-6">
  <div class="flex justify-between items-center">
    <div>
      ...
      <p class="text-[oklch(0.7107 0.0351 256.79)] mt-2">Manage your portfolio projects and apps.</p>
    </div>
  ...
  <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
    {#each data.projects as item}
      <div class="card flex flex-col md:flex-row group relative overflow-hidden">
        ...
        <div class="w-full md:w-48 h-48 md:h-auto bg-[oklch(0.2077 0.0398 265.75)] shrink-0 border-b md:border-b-0 md:border-r border-[oklch(0.3717 0.0392 257.29)] relative overflow-hidden">
          {#if item.image}
          ...
          {:else}
            <div class="w-full h-full flex items-center justify-center text-[oklch(0.7107 0.0351 256.79)]">No Image</div>
          {/if}
          {#if item.private}
  ...
  {#if data.projects.length === 0}
    <div class="card p-8 text-center text-[oklch(0.7107 0.0351 256.79)]">No projects found. Add some!</div>
  {/if}
</div>

{#if isModalOpen}
  <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
    <div class="bg-[oklch(0.2795 0.0368 260.03)] border border-[oklch(0.3717 0.0392 257.29)] rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
      ...
      <form method="POST" action="?/save" enctype="multipart/form-data" use:enhance={() => {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            <label for="project-id" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">ID (Unique)</label>
          ...
          <div class="space-y-2">
            <label for="project-name" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Name</label>
            <input id="project-name" type="text" name="name" bind:value={currentItem.name} class="input-field" required />
          </div>
          
          <div class="space-y-2 md:col-span-2">
            <label for="project-desc" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Description</label>
          ...
          <div class="space-y-2 md:col-span-2">
            <label for="project-tags" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Tags (comma separated)</label>
          ...
          <div class="space-y-2">
            <label for="project-github" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">GitHub URL</label>
            <input id="project-github" type="text" name="github" bind:value={currentItem.github} class="input-field" />
          </div>
          
          <div class="space-y-2">
            <label for="project-url" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Website URL</label>
            <input id="project-url" type="text" name="url" bind:value={currentItem.url} class="input-field" />
          </div>
          
          <div class="space-y-2">
            <label for="project-image" class="block text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Image (Paste anywhere to upload)</label>
            <input id="project-image" type="file" name="imageFile" bind:this={fileInput} accept="image/*" class="w-full text-sm text-[oklch(0.7107 0.0351 256.79)] file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-500/10 file:text-blue-400 hover:file:bg-blue-500/20" />
          ...
          <div class="space-y-2 flex items-center pt-8">
            <label class="flex items-center space-x-3 cursor-pointer">
              <input type="checkbox" name="private" value="true" checked={currentItem.private} class="w-5 h-5 rounded border-[oklch(0.3717 0.0392 257.29)] bg-[oklch(0.2077 0.0398 265.75)] text-blue-500 focus:ring-blue-500 focus:ring-offset-[oklch(0.2795 0.0368 260.03)]" />
              <span class="text-sm font-medium text-[oklch(0.7107 0.0351 256.79)]">Private Project</span>
            </label>
          </div>
        </div>
