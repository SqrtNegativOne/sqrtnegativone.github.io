...
{#if isSearchModalOpen}
  <div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[60]" data-modal-kind="search" role="dialog" aria-modal="true" data-modal="true" tabindex="-1">
    <div class="bg-[oklch(0.2103_0.0059_285.89)] border border-[oklch(0.2739_0.0055_286.03)] rounded shadow-2xl w-[95vw] max-w-7xl flex flex-col overflow-hidden">
      <div class="p-6 border-b border-[oklch(0.2739_0.0055_286.03)] bg-[oklch(0.1603_0.0059_285.89)] flex justify-between items-center shrink-0">
        ...
        <button aria-label="Close modal" onclick={() => isSearchModalOpen = false} class="text-[oklch(0.7107_0.0351_256.79)] hover:text-white cursor-pointer">
      ...
      <div class="overflow-x-auto overflow-y-hidden p-6 flex gap-6">
        {#each searchResults as result, i (i)}
          <div 
            class="flex flex-col gap-2 p-3 rounded border border-[oklch(0.2739_0.0055_286.03)] bg-[oklch(0.1408_0.0044_285.82)] hover:bg-[oklch(0.25_0.005_285.89)] cursor-pointer transition-colors w-64 shrink-0 group"
          ...
          >
            {#if result.coverUrl}
              <div class="cover-container w-full shrink-0 flex flex-col gap-1 items-center">
                <div class="w-full aspect-[2/3] bg-black/40 rounded overflow-hidden">
            ...
            {:else}
              <div class="w-full aspect-[2/3] bg-black/40 rounded flex items-center justify-center text-[oklch(0.7107_0.0351_256.79)] shrink-0 mb-2">
                No Cover
              </div>
            {/if}
